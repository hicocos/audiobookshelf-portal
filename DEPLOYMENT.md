...
- `moyin-api`：Portal API 与内置定时任务，仅监听 `127.0.0.1:8019`。
- `moyin-web`：Next.js，仅监听 `127.0.0.1:3009`。
- `moyin-bot`：Telegram polling，无宿主端口。

Portal 自有服务缩减为三个容器。到期同步、活跃度、群组宽限、通知、对账和数据保留
任务由 API 生命周期内的调度器执行；`/data/scheduler.lock` 保证多个 API 进程中仅一个
实例执行任务，API 关闭时会等待当前任务完成并释放锁。
...
`up` 自动写入统一的构建版本、提交号和构建时间，构建并启动 ABS 与三个 Portal 服务。
...
`up` 使用 `--remove-orphans`，从旧版本升级时会删除已被 API 调度器取代的
`moyin-worker` 容器。旧 `.env.worker` 不再使用，可在确认升级正常后自行归档或删除。
...
- `.env.api`：Portal API、内置调度器的数据库与 ABS 管理 API key，以及 JWT、内部 Bot 鉴权和 `TELEGRAM_ADMIN_IDS`。
...
```bash
for n in moyin-api moyin-bot; do
  echo "[$n]"
...
Portal 使用 ABS 2.35.1 的托管 API key，而不是把 root 用户登录口令提供给 Portal。API key 名称为 `moyin-portal`，仅 API 持有。
...
```bash
docker compose up -d --no-build moyin-api
```
