...
import { ArrowRight, Eye, EyeOff, KeyRound, LifeBuoy, Lock, Sparkles, User } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  async function submit() {
    ...
    if (password !== confirmPassword) { setMessage('两次输入的密码不一致，请重新确认。'); return; }
  ...
  const notes = ['用户名建议使用英文、数字或下划线。', '邀请码仅限本人使用，请妥善保存。', '注册后必须绑定 Telegram Bot，绑定完成后账号才会启用。'];
  return (
      ...
      <section className="anime-panel grid w-full max-w-5xl overflow-hidden lg:grid-cols-[.96fr_1.04fr]">
        <div className="relative hidden min-h-[620px] flex-col justify-between p-10 lg:flex">
          ...
          <div className="relative z-10">
            ...
            <p className="lede mt-5 max-w-sm">使用管理员发放的邀请码创建账号。注册后请立即绑定 Telegram Bot，绑定完成后才会启用收听权限。</p>
          </div>
        ...
        <div className="p-7 sm:p-11">
          ...
          <p className="lede mt-3 text-sm">填写账号信息和邀请码后，还需要绑定 Telegram Bot 才能启用账号。</p>
          <div className="mt-5"><StatusNote tone="warning">注册成功后账号处于“待启用”状态。请进入账号中心生成绑定码，并在 Telegram Bot 中完成绑定；绑定前无法使用听书客户端。</StatusNote></div>
          ...
          <label className="mt-7 block"><span className="mb-2 flex items-center gap-2 text-sm font-black"><User size={15} /> 用户名</span><input className="field" placeholder="3–18 位，例如 moyin_user" minLength={3} maxLength={18} value={username} onChange={(e) => setUsername(e.target.value)} disabled={!registrationEnabled || loading} /></label>
          <label className="mt-4 block"><span className="mb-2 flex items-center gap-2 text-sm font-black"><Lock size={15} /> 密码</span><input className="field" type={showPassword ? 'text' : 'password'} autoComplete="new-password" placeholder={`${minPasswordLength}–18 位`} minLength={minPasswordLength} maxLength={18} value={password} onChange={(e) => setPassword(e.target.value)} disabled={!registrationEnabled || loading} /></label>
          <label className="mt-4 block"><span className="mb-2 flex items-center gap-2 text-sm font-black"><Lock size={15} /> 确认密码</span><input className="field" type={showPassword ? 'text' : 'password'} autoComplete="new-password" maxLength={18} value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={!registrationEnabled || loading} /></label>
          <button type="button" className="mt-3 inline-flex items-center gap-2 text-sm font-black text-[var(--primary)]" aria-label={showPassword ? '隐藏密码' : '显示密码'} onClick={() => setShowPassword((value) => !value)}>{showPassword ? <EyeOff size={15} /> : <Eye size={15} />}{showPassword ? '隐藏密码' : '显示密码'}</button>
          ...
          <p className="mt-6 text-center text-sm text-[var(--muted-foreground)]">注册后会自动进入账号中心，请按页面引导立即绑定 Telegram Bot。</p>
        </div>
      </section>
    </ShellBackdrop>
  );
