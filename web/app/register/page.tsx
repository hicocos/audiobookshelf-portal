    ...
    <ShellBackdrop className="grid place-items-center px-4 py-8">
      ...
      <section className="anime-panel grid w-full max-w-5xl overflow-hidden lg:grid-cols-[.96fr_1.04fr]">
        ...
        <div className="p-7 sm:p-11">
          ...
          <label className="mt-4 block"><span className="mb-2 flex items-center gap-2 text-sm font-black"><Lock size={15} /> 密码</span><input className="field" type={showPassword ? 'text' : 'password'} autoComplete="new-password" placeholder={`至少 ${minPasswordLength} 位`} minLength={minPasswordLength} value={password} onChange={(e) => setPassword(e.target.value)} disabled={!registrationEnabled || loading} /></label>
          <label className="mt-4 block"><span className="mb-2 flex items-center gap-2 text-sm font-black"><Lock size={15} /> 确认密码</span><input className="field" type={showPassword ? 'text' : 'password'} autoComplete="new-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} disabled={!registrationEnabled || loading} /></label>
