...
async def run_once() -> dict[str, int]:
    ...
    with Session(engine) as session:
        async with AudiobookshelfClient(
            ...
            operations = public_settings.get("operations") if isinstance(public_settings.get("operations"), dict) else {}
            ...
            if inactive.get("enabled"):
                update_public_settings(session, {
                    "operations": {
                        **operations,
                        "lastInactivityCheckAt": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
